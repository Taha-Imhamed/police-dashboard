//////////////////////////////////////////////////////////
//this one for the plate 


// Show/hide sections
document.getElementById('register-weapon-btn').addEventListener('click', () => {
    document.getElementById('register-weapon-form-section').classList.remove('hidden');
    document.getElementById('view-weapons-section').classList.add('hidden');
  });
  
  document.getElementById('view-weapons-btn').addEventListener('click', async () => {
    document.getElementById('view-weapons-section').classList.remove('hidden');
    document.getElementById('register-weapon-form-section').classList.add('hidden');
  
    await loadWeapons();
  });
  
  // Handle weapon registration form submission
  document.getElementById('register-weapon-form').addEventListener('submit', async function (event) {
    event.preventDefault();
  
    const formData = {
      citizenId: document.getElementById('citizen-id').value.trim(),
    ...
    if (!formData.citizenId || !formData.weaponType || !formData.serialNumber || !formData.purchaseDate) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error('Failed to register weapon.');
      }
  
      alert('Weapon registered successfully!');
      this.reset();
      await loadWeapons();
    } catch (error) {
      console.error(error);
      alert('An error occurred while registering the weapon.');
    }
  });
  
  // Load registered weapons
  async function loadWeapons() {
    try {
      ...
      weapons.forEach((weapon) => {
        ...
        row.innerHTML = `
          <td>${weapon.citizen_id}</td>
          <td>${weapon.citizen_name}</td>
          <td>${weapon.weapon_type}</td>
          <td>${weapon.serial_number}</td>
          <td>${weapon.purchase_date}</td>
        `;
        tbody.appendChild(row);
      });
    } catch (error) {
      console.error(error);
      alert('An error occurred while fetching weapons.');
    }
  }
  ////////////////////////////////////////////////////////////
  ////////////////////////////////////////////////////////////
 //// this one for the wepons registration 
 // scripts.js
